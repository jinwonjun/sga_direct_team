...
void VertexBlending::Init()
{
	...
	g_pDevice->CreateVertexBuffer(m_numVertices * sizeof(VERTEX_BLENDING), 0,
		VERTEX_BLENDING::FVF, D3DPOOL_DEFAULT, &m_pVB, NULL);
	...
	for (int i = 0; i < numHalfVertices; i++)
	{
		...
		pV[2 * i +0] = VERTEX_BLENDING(D3DXVECTOR3(cos(theta), 1.0, sin(theta)),
			weight1, 0x0000001, D3DXVECTOR2(i / (float)(numHalfVertices - 1), 0));

		pV[2 * i + 1] = VERTEX_BLENDING(D3DXVECTOR3(cos(theta), 1.0, sin(theta)),
			weight2, 0x0000100, D3DXVECTOR2(i / (float)(numHalfVertices - 1), 1));

	}

	m_pVB->Unlock();
	m_pTex = g_pTextureManager->GetTexture("ham1.png");
...
void VertexBlending::Update()
{
	...
	D3DXMatrixScaling(&m_mat2, 0.5 * sin(d / 1000.0f), 0, 0.5 *cos(d / 1000.0));
...
void VertexBlending::Render()
{
	...
	g_pDevice->SetRenderState(D3DRS_VERTEXBLEND, D3DVBF_3WEIGHTS); // 4개 가중치임
	...
	g_pDevice->SetRenderState(D3DRS_VERTEXBLEND, D3DVBF_DISABLE); // 4개 가중치임
