...
void PortalEffect::Init()
{
	...
	//생성 위치 조절
	D3DXMATRIXA16 matT;
	D3DXMatrixTranslation(&matT, 100, -50, 80);
...
void PortalEffect::Render()
{
	...
	if (static_cast <EnemyManager *>(g_pObjMgr->FindObjectByTag(TAG_ENEMY))->GetVecEnemy().size() == 0
		&& static_cast <ObjMap *>(g_pObjMgr->FindObjectByTag(TAG_OBJMAP))->GetMapChangeSignal() == false)
	{
		isPortal = true;
	}
