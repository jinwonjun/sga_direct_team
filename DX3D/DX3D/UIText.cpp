	...
	: IUIObject(pSptrite, uiTag),m_pFont(pFont),m_drawTextFormat(DT_CENTER | DT_VCENTER)//DT_ 종류들은 텍스트 기본 정렬 방식
{
	...
	//if (pText == NULL)
	//{
	//	pText = new UIText(pFont, m_pSprite);
	//}
	//DT_CENTER
}

UIText::UIText(LPD3DXFONT pFont, LPD3DXSPRITE pSptrite, char a, int uiTag)
	: IUIObject(pSptrite, uiTag), m_pFont(pFont), m_drawTextFormat(DT_LEFT | DT_VCENTER)
{
	RenderingOn = false;
